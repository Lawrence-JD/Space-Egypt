...
public:
	...
	UPROPERTY(Category="Savior 3", EditAnywhere, BlueprintReadWrite, meta=(DisplayName="UI Slot Picker"))
	TSubclassOf<USlotPickerWidget>SlotPickerWidget;
	//
	//
	/** Event fired when a Load-Screen is created. */
	UPROPERTY(Category="Savior 3", BlueprintAssignable, meta=(DisplayName="[SAVIOR] On Begin Load-Screen"))
	FHUD_LoadScreen OnBeginLoadScreen;
	//
	/** Event fired when a Load-Screen is destroyed. */
	UPROPERTY(Category="Savior 3", BlueprintAssignable, meta=(DisplayName="[SAVIOR] On Finish Load-Screen"))
	FHUD_LoadScreen OnFinishLoadScreen;
public:
	/// :: FUNCTIONS ::
	///
	/** Event called when a Load-Screen is shown. */
	UFUNCTION(Category="Savior 3", BlueprintImplementableEvent, meta=(DisplayName="[SAVIOR] On Begin Load-Screen"))
	void OnBeganLoadScreen();
	//
	/** Event called when a Load-Screen is hidden. */
	UFUNCTION(Category="Savior 3", BlueprintImplementableEvent, meta=(DisplayName="[SAVIOR] On Finish Load-Screen"))
	void OnFinishedLoadScreen();
	//
	//
	/** Adds an auto-generated Save Slot Picker UI to the Viewport. */
	UFUNCTION(Category="Savior 3", BlueprintNativeEvent, BlueprintCallable, meta=(DisplayName="[SAVIOR] Show Slots UI"))
	void ShowSlotPickerHUD(const ESlotPickerMode PickerMode);
	//
	/** Removes auto-generated Slot Picker UI from the Viewport. */
	UFUNCTION(Category="Savior 3", BlueprintNativeEvent, BlueprintCallable, meta=(DisplayName="[SAVIOR] Hide Slots UI"))
	void HideSlotPickerHUD();
	//
	//
	/** Forces a loading screen canvas to draw on HUD. */
	UFUNCTION(Category="Savior 3", BlueprintCallable, meta=(DisplayName="[SAVIOR] Invoke Load Screen (Blur)"))
	...
	//
	/** Forces a loading screen canvas to draw on HUD. */
	UFUNCTION(Category="Savior 3", BlueprintCallable, meta=(DisplayName="[SAVIOR] Invoke Load Screen (Splash)"))
	...
	//
	/** Forces a loading screen canvas to draw on HUD. */
	UFUNCTION(Category="Savior 3", BlueprintCallable, meta=(DisplayName="[SAVIOR] Invoke Load Screen (Video)"))
	...
	UFUNCTION(Category="Savior 3", BlueprintCallable, meta=(DisplayName="[SAVIOR] Remove Load Screen"))
