	...
	if (CDO!=nullptr) {
		...
		if (ACT!=nullptr) {
			...
			if (auto*Root=ACT->GetRootComponent()) {
				...
				} else {ACT->SetActorLocation(Data.Location);}
			}///
			//
...
FMaterialRecord Reflector::CaptureMaterialSnapshot(UMaterialInstanceDynamic* Instance) {
	...
	//
	TArray<FGuid>ScalarID;	TArray<FMaterialParameterInfo>ScalarInfo;
	TArray<FGuid>VectorID;	TArray<FMaterialParameterInfo>VectorInfo;
	TArray<FGuid>TextureID;	TArray<FMaterialParameterInfo>TextureInfo;
	//
...
const FName Reflector::MakeObjectID(UObject* OBJ, bool FullPath) {
	if (!OBJ||OBJ->IsPendingKill()) {return TEXT("ERROR:ID");}
	//
	FString OwnerID;
...
const FName Reflector::MakeActorID(AActor* Actor, bool FullPath) {
	if (!Actor||Actor->IsPendingKill()) {return TEXT("ERROR:ID");}
	//
	FString NetworkID;
...
const FName Reflector::MakeComponentID(UActorComponent* CMP, bool FullPath) {
	if (!CMP||CMP->IsPendingKill()) {return TEXT("ERROR:ID");}
	//
	FString OwnerID;
