...
void LOG_SV(const bool Debug, const ESeverity Severity, const TCHAR* Message) {
	...
	switch (Severity) {
		case ESeverity::Info:
			UE_LOG(Saviors,Log,TEXT("{S}:: %s"),Message); break;
		case ESeverity::Warning:
			UE_LOG(Saviors,Warning,TEXT("{S}:: %s"),Message); break;
		case ESeverity::Error:
			UE_LOG(Saviors,Error,TEXT("{S}:: %s"),Message); break;
		case ESeverity::Fatal:
			UE_LOG(Saviors,Fatal,TEXT("{S}:: %s"),Message); break;
	default: break;}
}
...
void LOG_SV(const bool Debug, const ESeverity Severity, const FName Message) {
	...
	switch (Severity) {
		case ESeverity::Info:
			UE_LOG(Saviors,Log,TEXT("{S}:: %s"),*Message.ToString()); break;
		case ESeverity::Warning:
			UE_LOG(Saviors,Warning,TEXT("{S}:: %s"),*Message.ToString()); break;
		case ESeverity::Error:
			UE_LOG(Saviors,Error,TEXT("{S}:: %s"),*Message.ToString()); break;
		case ESeverity::Fatal:
			UE_LOG(Saviors,Fatal,TEXT("{S}:: %s"),*Message.ToString()); break;
	default: break;}
}
...
const bool Reflector::IsSafeToRespawnInstance(const UObject* CDO) {
...
void Reflector::RespawnActorFromData(UWorld* World, const TSet<TSubclassOf<UObject>>RespawnScope, const FSaviorRecord Data) {
	check(IsInGameThread()); if (World==nullptr) {return;}
	//
	...
	if (CDO!=nullptr) {
		...
		if (ACT!=nullptr) {
			...
			ACT->SetActorScale3D(Data.Scale);
...
void Reflector::RespawnComponentFromData(UWorld* World, const TSet<TSubclassOf<UObject>>RespawnScope, const FSaviorRecord Data) {
	...
	for (TObjectIterator<UActorComponent>OBJ; OBJ; ++OBJ) {
		if ((*OBJ)->HasAnyFlags(RF_DefaultSubObject|RF_ArchetypeObject|RF_BeginDestroyed)) {continue;}
...
///	(C) 2018 - Bruno Xavier B. Leite
void Reflector::JSONToFProperty(USavior2* Savior, TSharedPtr<FJsonObject> &JSON, const FString &PID, FObjectProperty* Property, UObject* Container) {
	check(Property); check(Container);
	//
	auto ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
	...
	if ((CDO!=nullptr)&&CDO->IsA(AActor::StaticClass())) {
		...
		for (TObjectIterator<UActorComponent>CMP; CMP; ++CMP) {
			...
			if ((*CMP)->HasAnyFlags(RF_ClassDefaultObject|RF_ArchetypeObject|RF_BeginDestroyed)) {continue;}
			//
...
///	(C) 2018 - Bruno Xavier B. Leite
void Reflector::JSONToFProperty(USavior2* Savior, TSharedPtr<FJsonObject> &JSON, const FString &PID, FArrayProperty* Property, UObject* Container) {
	check(Property); check(Container);
	//
...
///	(C) 2018 - Bruno Xavier B. Leite
void Reflector::JSONToFProperty(USavior2* Savior, TSharedPtr<FJsonObject> &JSON, const FString &PID, FSetProperty* Property, UObject* Container) {
	check(Property); check(Container);
	//
...
///	(C) 2018 - Bruno Xavier B. Leite
void Reflector::JSONToFProperty(USavior2* Savior, TSharedPtr<FJsonObject> &JSON, const FString &PID, FMapProperty* Property, UObject* Container) {
	check(Property); check(Container);
	//
	auto MapPtr = Property->ContainerPtrToValuePtr<void>(Container);
...
///	(C) 2018 - Bruno Xavier B. Leite
const bool Reflector::IsMatchingSGUID(UObject* Target, UObject* ComparedTo) {
...
///	(C) 2018 - Bruno Xavier B. Leite
const bool Reflector::IsMatchingSGUID(const FGuid &SGUID, UObject* Target) {
...
///	(C) 2018 - Bruno Xavier B. Leite
const bool Reflector::GetPlayerNetworkID(const APlayerController* PlayerController, FString &ID, const bool AppendPort) {
...
const FName Reflector::MakeObjectID(UObject* OBJ, bool FullPath) {
	...
	if (Owner) {
		...
		OwnerID = (FullPath) ? Owner->GetFullName() : Owner->GetFName().ToString();
		//
	...
	FString ID = (FullPath) ? OBJ->GetFullName() : OBJ->GetFName().ToString();
	//
...
const FName Reflector::MakeActorID(AActor* Actor, bool FullPath) {
	...
	FString ID = (FullPath) ? Actor->GetFullName() : Actor->GetFName().ToString();
	//
	if (ID.Contains(TEXT("_C"))) {
		ID.Split(TEXT("_C"),&ID,nullptr,ESearchCase::CaseSensitive,ESearchDir::FromEnd);
	} if (!GUID.IsEmpty()) {
...
const FName Reflector::MakeComponentID(UActorComponent* CMP, bool FullPath) {
	...
	FString ID = (FullPath) ? CMP->GetFullName() : CMP->GetFName().ToString();
	//
	if (ID.Contains(TEXT("_C"))) {
		ID.Split(TEXT("_C"),&ID,nullptr,ESearchCase::CaseSensitive,ESearchDir::FromEnd);
	} if (!GUID.IsEmpty()) {
